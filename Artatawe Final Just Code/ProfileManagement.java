...
public class ProfileManagement {
	...
	public void save() {
		...
		try {
			...
			for (int i = 0; i < profiles.size(); i++) {
				...
				line += (profiles.get(i).getProfilePicPath()) + ",";
	...
	public void load() {
		...
		try {
			...
			while (fileReader.hasNext()) {
				...
				profiles.add(new Profile(lastName, firstName, userName, phoneNo, address, postCode, image));
			}
			fileReader.close();
		} catch (Exception e) {
			// file io error
	...
	public void addProfile(String lastName, String firstName, String userName, String phoneNo, String adress,
			String postCode, String profilePic) {
		if (checkUniqueName(userName)) {
			if (phoneNo.length() != 11 && phoneNo.charAt(0) != 0 && phoneNo.charAt(1) != 7) {
				...
				Profile profile = new Profile(lastName, firstName, userName, phoneNo, adress, postCode, profilePic);
