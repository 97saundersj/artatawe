		...
		try {
			...
			while (fileReader.hasNext()) {
				...
				if(type.equals("Artwork")){
					artworks.add(new Artwork(title, description, imagePaths.get(0), null, 
							Integer.parseInt(yearOfCreation), Integer.parseInt(reservePrice), 
							Integer.parseInt(noOfBids), Integer.parseInt(width), Integer.parseInt(height),bidStart
							));
					
				}
				else{
					artworks.add(new Sculpture(title, description, imagePaths.get(0), null, 
							Integer.parseInt(yearOfCreation), Integer.parseInt(reservePrice), 
							Integer.parseInt(noOfBids), Integer.parseInt(width),Integer.parseInt(height), bidStart,Integer.parseInt(depth),material
							));
				}
				for(int i = 0; i < imagePaths.size(); i++) {
					artworks.get(artworks.size() - 1).addImageToArrayList(imagePaths.get(0));
				}
